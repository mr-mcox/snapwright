...
from snapwright.evolution.diff import diff_channels, diff_snapshots
...
class TestSnapshotDiffChannelFiltering:
    ...
    def test_unnamed_channels_excluded(self):
        base_snap = self._make_snap(
            {"1": make_channel(name="Kick"), "2": make_channel(name="")}
        )
        target_snap = self._make_snap(
            {
                "1": make_channel(name="Kick", fdr=0.0),
                "2": make_channel(name="", fdr=0.0),
            }
        )
