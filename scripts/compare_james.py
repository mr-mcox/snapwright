...
print(
    f"{'DCA':<6} {'Base name':<15} {'Base fdr':>10} {'James name':<15} {'James fdr':>10}"
)
print("-" * 60)
for n in [str(i) for i in range(1, 17)]:
    ...
    if b_name or j_name:
        print(
            f"dca{int(n):02d}: {b_name:<15} {b_fdr:>10.1f}   {j_name:<15} {j_fdr:>10.1f}"
        )
...
for n in [str(i) for i in range(1, 9)]:
    ...
    print(
        f"mgrp{int(n):02d}: base={b_name!r} mute={b_mute}  james={j_name!r} mute={j_mute}"
    )

print()
print("=== CHANNEL NAMES: Base vs James ===")
print(
    f"{'Ch':<5} {'Base name':<20} {'Base src':<12} {'James name':<20} {'James src':<12}"
)
print("-" * 72)
