...
for section, indices in ae_sections:
    ...
    header = f"{'idx':<5} {'Init':<18} {'Base':<20} " + " ".join(f"{t:<18}" for t in teams)
    print(header)
    print("-"*160)
    for i in indices:
        key = str(i)
        init_name = init.get("ae_data", {}).get(section, {}).get(key, {}).get("name", "")
        base_name = base.get("ae_data", {}).get(section, {}).get(key, {}).get("name", "")
        names = []
        all_same_across_teams = True
        first_team = None
        for t in teams:
            n = snaps[t].get("ae_data", {}).get(section, {}).get(key, {}).get("name", "")
            names.append(n)
            if first_team is None:
                first_team = n
            elif n != first_team:
                all_same_across_teams = False
        row = f"{i:<5} {str(init_name):<18} {str(base_name):<20} " + " ".join(f"{str(n):<18}" for n in names)
