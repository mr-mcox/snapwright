...
print("Legend:")
print(
    "  [INFRA-NAMED]  = same non-empty name across ALL teams -> likely infrastructure"
)
print("  [INFRA-EMPTY]  = empty across all teams -> likely unused")
print(
    "  [SEMI-INFRA]   = named in some teams with same name, empty in others -> partially managed"
)
...
for i in range(1, 41):
    key = str(i)
    names_by_team = {
        t: snaps[t].get("ae_data", {}).get("ch", {}).get(key, {}).get("name", "")
        for t in teams
    }
