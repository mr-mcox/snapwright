...
for n in [str(i) for i in range(1, 9)]:
    ...
    main1 = main.get("1", {})
    print(
        f"  aux{int(n):02d}: name={repr(name)} fdr={fdr:.1f} col={col} icon={icon} mute={mute} main1={main1}"
    )
...
for n in [str(i) for i in range(1, 5)]:
    ...
    print(
        f"  main{int(n):02d}: name={repr(name)} fdr={fdr:.1f} mute={mute} icon={icon} eq_on={eq_on} dyn_on={dyn_on}"
    )
...
talkA = talk.get("A", {})
print(
    f"  talk.A buses: {[(k, v) for k, v in talkA.items() if v and k.startswith('B')]}"
)
