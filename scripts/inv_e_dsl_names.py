...
for ch_num, musician in sorted(james_assembly["channels"].items()):
    snap_name = (
        james.get("ae_data", {}).get("ch", {}).get(str(ch_num), {}).get("name", "")
    )
    ...
    for inh in inherits:
        ...
        if stem in musician_names:
            ...
            break
    print(
        f"  ch {ch_num:2d} = {musician:<20} DSL name='{dsl_name or '(no name)'}' snap='{snap_name}'"
    )
...
for bus_num, logical_name in sorted(buses.items()):
    snap_name = (
        james.get("ae_data", {}).get("bus", {}).get(str(bus_num), {}).get("name", "")
    )
    base_name = (
        base.get("ae_data", {}).get("bus", {}).get(str(bus_num), {}).get("name", "")
    )
    print(
        f"  bus {bus_num:2d} = '{logical_name:<20}' snap='{snap_name}' base='{base_name}'"
    )
...
for i in range(1, 41):
    ...
    if snap_name and i not in dsl_channels:
        base_name = (
            base.get("ae_data", {}).get("ch", {}).get(str(i), {}).get("name", "")
        )
        print(
            f"  ch {i:2d}: snap='{snap_name}' base='{base_name}' (source: {'Base.snap' if base_name else 'manual/unknown'})"
        )
...
for ch_num, musician in sorted(james_assembly["channels"].items()):
    ...
    if dsl_name and snap_name and dsl_name != snap_name:
        print(
            f"  ch {ch_num:2d} ({musician}): DSL='{dsl_name}' snap='{snap_name}' MISMATCH"
        )
    elif not dsl_name:
